...
def _reml_random_effects(yi, vi, max_iter=100, tol=1e-8):
    """Paule-Mandel iterative tau^2 estimator (not true REML; used as Python-side
    approximation). R simulation uses metafor::rma(method='REML') which is authoritative."""
    y = np.asarray(yi, dtype=np.float64)
    v = np.maximum(np.asarray(vi, dtype=np.float64), 1e-15)
    k = len(y)
